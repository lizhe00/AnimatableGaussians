...
//   const float w0 = 1.0f - w1 - w2;
  ...
  float3 vec1 = v1 - p;
  float3 vec2 = v2 - p;
  float area0 = norm(cross(vec1, vec2));
  float area1 = norm(cross(vec2, vec0));
  float area2 = norm(cross(vec0, vec1));
...
//   bool is_inside = x_in && y_in && z_in;
  bool is_inside = pt_in_triangle(v0, v1, v2, p);
...
//   }
  ...
  if (is_inside){
    const float t = dot(v0 - p, normal);
    closest_pt = p + t * normal;
    dist = dot(p - closest_pt, p - closest_pt);
  }else{
    ...
    dist = dist * dist;  // squared distance
  }
